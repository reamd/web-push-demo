...
self.addEventListener("push", (event) => {
  ...
  try {
    ...
    event.waitUntil(
      self.registration
        .showNotification("标题示例123", {
          body: "通知正文内容", // 避免与标题重复‌:ml-citation{ref="2" data="citationList"}
          tag: "unique-tag", // 用于分组或替换同类型通知
          data: { key: "value" }, // 附加数据，可在通知点击事件中获取
        })
        .catch((err) => {
          console.error("推送异常666:", err);
        })
    );
    // event.waitUntil(
